...
/**
 * 커스텀 signout 엔드포인트
 * NextAuth의 기본 signout이 쿠키를 제대로 삭제하지 않는 경우를 대비
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (session) {
      console.log("[Signout API] Logging out user:", session.user?.id);
    }
    ...
    console.log("[Signout API] Cookies deletion headers set:", setCookieHeaders.length, "headers");
    return response;
  } catch (error) {
